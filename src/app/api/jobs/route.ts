...
export async function GET(req: NextRequest) {
  try {
    ...
    // Get user's blacklisted companies
    const dbUser = await User.findById(user.userId).select("blacklistedCompanies")
    const blacklist: string[] = dbUser?.blacklistedCompanies || []

    // Build blacklist regex array once — reused in both paths
    const blacklistFilter = blacklist.length > 0
      ? { company: { $not: { $in: blacklist.map((c) => new RegExp(`^${c}$`, "i")) } } }
      : {}
    ...
    if (search.trim()) {
      ...
      // Post-search filters including blacklist
      const matchStage: any = { ...blacklistFilter }
      ...
      const query: any = { ...blacklistFilter }
    ...
    if (error?.message?.includes("$search") || error?.codeName === "CommandFailed") {
      ...
      try {
        ...
        // Re-fetch blacklist for fallback path
        const token   = cookies().get("token")?.value
        const user    = token ? await verifyToken(token) : null
        const dbUser  = user ? await User.findById(user.userId).select("blacklistedCompanies") : null
        const blacklist: string[] = dbUser?.blacklistedCompanies || []

        const blacklistFilter = blacklist.length > 0
          ? { company: { $not: { $in: blacklist.map((c: string) => new RegExp(`^${c}$`, "i")) } } }
          : {}
...
//     // Get user's blacklisted companies
//     const dbUser = await User.findById(user.userId).select("blacklistedCompanies")
//     const blacklist = dbUser?.blacklistedCompanies || []
